...
<div class="bg-neutral-800 py-2 flex font-sans px-2">
    ...
    <div class="flex px-2 text-clip gap-2 overflow-hidden justify-evenly flex-1">
        {#if isEditing}
            <form on:submit={updateProfile} class="flex overflow-auto flex-1">
                <input class="text-clip overflow-hidden w-0 flex-1" autofocus type="text" bind:value={username} on:blur={() => isEditing = false}/>
        ...
        {/if}
        <svg on:click={logout} title="Logout" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5 text-white stroke-2 self-center ml-1 cursor-grab">
