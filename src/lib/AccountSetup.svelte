...
<div class="bg-neutral-800 py-2 flex font-sans px-2">
    ...
    <div class="flex px-2 text-clip gap-2 overflow-hidden">
        {#if isEditing}
            <form on:submit={updateProfile} class="flex overflow-auto max-width-fit">
                <input class="text-clip overflow-hidden" autofocus type="text" bind:value={username} on:blur={() => isEditing = false}/>
                
                <svg title="Cancel" on:click={() => isEditing = false} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-6 h-6 cursor-grab">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                </svg>
            </form>
        {:else}
            <div class="text-lg text-white text-clip ml-2 overflow-hidden">{username}</div>
